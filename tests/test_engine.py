...
class TestEngine:
    ...
    def test_rsa_sign(self, tmpdir):
        ...
        data = b'\x00' * 8192
        p = tmpdir.join("test_sign_data")
        p.write_binary(data)

        for k in ('sha1', 'sha256', 'sha384', 'sha512'):

            sig = pkey.sign(data, engine.engine_padding_pkcs1(), engine.engine_hashes(k))
            assert len(sig) in (128, 256, 384, 512)

            p_sig = tmpdir.join("test_sign_sig")
            p_sig.write_binary(sig)

            proc = subprocess.run([
                'openssl', 'dgst', f'-{k}', '-engine', 'pkcs11', '-keyform',
                'engine', '-verify', alias, '-signature',
                tmpdir.join('test_sign_sig'),
                tmpdir.join('test_sign_data')
            ])

            assert proc.returncode == 0

            pubkey.verify(sig, data, engine.engine_padding_pkcs1(), engine.engine_hashes(k))

        engine.engine_finish(e)

    def test_pss_sign(self, tmpdir):
        ...
        data = b'\x00' * 8192
        p = tmpdir.join("test_pss_sign_data")
        p.write_binary(data)
        ...
        for k in ('sha256:222', 'sha256:96', 'sha384:206', 'sha512:64'):
            ...
            p_sig = tmpdir.join("test_pss_sign_sig")
            p_sig.write_binary(sig)
            ...
            proc = subprocess.run([
                ...
                '-signature', tmpdir.join('test_pss_sign_sig'),
                tmpdir.join('test_pss_sign_data')
            ])
    ...
    def test_ec_sign(self, tmpdir):
        ...
        data = b'\x00' * 8192
        p = tmpdir.join("test_ec_sign_data")
        p.write_binary(data)

        for k in ('sha256', 'sha384', 'sha512'):
            sig = pkey.sign(data, engine.ecdsa_with_hash(k))
            p_sig = tmpdir.join("test_ec_sign_sig")
            p_sig.write_binary(sig)

            proc = subprocess.run([
                'openssl', 'dgst', f'-{k}', '-engine', 'pkcs11', '-keyform',
                'engine', '-verify', alias, '-signature',
                tmpdir.join('test_ec_sign_sig'),
                tmpdir.join('test_ec_sign_data')
            ])
    ...
    def test_rsa_encrypt(self, tmpdir):
        ...
        data = os.urandom(128)
        p = tmpdir.join("test_encrypt_data")
        p.write_binary(data)
    ...
    def test_engine_sign(self, tmpdir):
        ...
        for k in ('sha256', 'sha384', 'sha512'):
            ...
            p_sig = tmpdir.join(f'test_sign_sig-{k}')
            p_sig.write_binary(sig)
            ...
            proc = subprocess.run([
                ...
                '-keyform', 'engine',
                '-verify',  alias,
                '-signature', tmpdir.join(f'test_sign_sig-{k}'),
                tmpdir.join('test_sign_data')
            ])
    ...
    def test_engine_pss_sign(self, tmpdir):
        ...
        for k in ('sha256:64', 'sha384:-1', 'sha512:-2'):
            ...
            p_sig = tmpdir.join(f'test_pss_sign_sig-{k}')
            p_sig.write_binary(sig)
            ...
            proc = subprocess.run([
                ...
                '-verify',  alias,
                '-sigopt', 'rsa_padding_mode:pss',
                '-signature', tmpdir.join(f'test_pss_sign_sig-{k}'),
                tmpdir.join('test_pss_sign_data')
            ])
    ...
    def test_engine_encrypt(self, tmpdir):
        ...
        assert proc.returncode == 0
        recover = engine.engine_decrypt(pkey._evp_pkey, out, (engine.RSAPadding.RSA_PKCS1_PADDING,))
        assert data == recover

        out = engine.engine_encrypt(pubkey._evp_pkey, data, padding=(engine.RSAPadding.RSA_PKCS1_OAEP_PADDING, 'sha256', 'sha256'))
        ciphered =  tmpdir.join("test_encrypt_cipheroaep")
        ciphered.write_binary(out)
        plaintext =  tmpdir.join("test_encrypt_plainoaep")
        ...
        assert proc.returncode == 0
        recover = engine.engine_decrypt(pkey._evp_pkey, out, (engine.RSAPadding.RSA_PKCS1_OAEP_PADDING, 'sha256', 'sha256'))
        assert data == recover
